...
uint8_t Crypto::iter_key[10][BLOCK_SIZE];
...
char* Crypto::Encrypt(const char* data, int size)
{
    FILE* f_public = fopen(_public_key_file_name, "rb");
...
void Crypto::gost_Encrypt(const uint8_t* message, uint8_t* encrypted_message)
{
    memcpy(encrypted_message, message, BLOCK_SIZE);

    for(int i = 0; i < 9; ++i)
    {
        X(iter_key[i], encrypted_message, encrypted_message);
        S(encrypted_message, encrypted_message);
        L(encrypted_message, encrypted_message);
    }

    X(iter_key[9], encrypted_message, encrypted_message);   
}

void Crypto::gost_Decrypt(const uint8_t* encrypted_message, uint8_t* decrypted_message)
{
    memcpy(decrypted_message, encrypted_message, BLOCK_SIZE);

    X(iter_key[9], encrypted_message, decrypted_message);
    for(int i = 8; i >= 0; --i)
    {
        L_Reverse(decrypted_message, decrypted_message);
        S_Reverse(decrypted_message, decrypted_message);
        X(decrypted_message, iter_key[i], decrypted_message);
    }
}
...
void Crypto::ExpandKey(const uint8_t *key)
{
    int i;
